...
class PseudoSpikeDeterministic(torch.autograd.Function):
    ...
    def forward(ctx, input):
        return input.gt(ENCODER_DET_VTH).float()
...
class PseudoSpikeAtan(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        return input.gt(ATAN_VTH).float()

    @staticmethod
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone()
        return grad_input / (1 + (math.pi * (input - ATAN_VTH)).pow_(2))
...
class PseudoSpikeRect(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ctx.save_for_backward(input)
        return input.gt(RECT_VTH).float()

    @staticmethod
    def backward(ctx, grad_output):
        input, = ctx.saved_tensors
        grad_input = grad_output.clone()
        spike_pseudo_grad = (abs(input - RECT_VTH) < SPIKE_PSEUDO_GRAD_WINDOW)
...
class PopSpikeEncoderDeterministic(nn.Module):
    ...
    def forward(self, obs, batch_size):
        ...
        for step in range(self.spike_ts):
            ...
            pop_volt = pop_volt - pop_spikes[:, :, step] * ENCODER_DET_VTH
...
class PopSpikeEncoderAtan(nn.Module):
    ...
    def forward(self, obs, batch_size):
        ...
        for step in range(self.spike_ts):
            pop_volt = pop_volt + pop_act
            pop_spikes[:, :, step] = self.pseudo_spike(pop_volt)
            pop_volt = pop_volt - pop_spikes[:, :, step] * ATAN_VTH
...
class PopDecoder(nn.Module):
    ...
    def __init__(self, act_dim, pop_dim, spike_ts, decode, output_activation=nn.Tanh):
        super().__init__()
...
class SpikeMLP(nn.Module):
    ...
    def neuron_model(self, syn_func, pre_layer_output, current, volt, spike, conn_flag=False):
    ...
    def forward(self, in_pop_spikes, batch_size):
        ...
        for step in range(self.spike_ts):
            ...
            out_pop_states[0], out_pop_states[1], out_pop_states[2] = self.neuron_model(
                ...
                out_pop_states[0], out_pop_states[1], out_pop_states[2], True
            )
    ...
    def get_each_layer_fire_rate_and_forward(self, in_pop_spikes, batch_size):
        
        each_layer_fr = np.zeros(self.hidden_num + 3) if self.connections == 'intra' else np.zeros(self.hidden_num + 2)
        each_layer_fr[0] = in_pop_spikes.mean().item()
        ...
        for step in range(self.spike_ts):
            ...
            each_layer_fr[1] += hidden_states[0][2].mean().item() / self.spike_ts
            ...
            if self.hidden_num > 1:
                for layer in range(1, self.hidden_num):
                    ...
                    each_layer_fr[layer + 1] += hidden_states[layer][2].mean().item() / self.spike_ts
            ...
            each_layer_fr[self.hidden_num + 1] += out_pop_states[2].mean().item() / self.spike_ts
            if self.connections == 'intra' and step != self.spike_ts - 1:
                each_layer_fr[self.hidden_num + 2] += out_pop_states[2].mean().item() / self.spike_ts
        ...
        if 'fr' in self.decode:
            ...
            return out_pop_act, each_layer_fr

        return out_pop_spikes, each_layer_fr
...
class PopSpikeActor(nn.Module):
    ...
    def get_each_layer_fire_rate_and_forward(self, obs, batch_size):
        if 'pop' in self.encode:
            input_trains = self.encoder(obs, batch_size)
        else:
            input_trains = torch.zeros(batch_size, self.obs_dim, self.spike_ts, device=self.device)
        ...
        out_pop_activity, each_layer_fr = self.snn.get_each_layer_fire_rate_and_forward(input_trains, batch_size)

        if 'fr' in self.decode:
            return (self.act_limit * self.decoder(out_pop_activity)).numpy(), each_layer_fr
        ...
        return (self.act_limit * torch.tanh(mem)).numpy(), each_layer_fr