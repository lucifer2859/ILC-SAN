...
def test_mujoco_render(popsan_model_file, mean_var_file, env_fn,
    ...
    each_layer_avg_fr = np.zeros(len(hidden_sizes) + 3) if connections == 'intra' else np.zeros(len(hidden_sizes) + 2)
    ...
    while not (d or (ep_len == 1000)):
        # test_env.render()
        with torch.no_grad():
            o = torch.as_tensor(replay_buffer.normalize_obs(o), dtype=torch.float32, device=device)
            a, each_layer_fr = popsan.get_each_layer_fire_rate_and_forward(o, 1)
            o, r, d, _ = test_env.step(np.clip(a, -act_limit, act_limit))
            each_layer_avg_fr += each_layer_fr
        ep_len += 1

    each_layer_avg_fr /= ep_len

    return each_layer_avg_fr
...
if __name__ == '__main__':
    ...
    each_layer_avg_fr = [0., 0., 0., 0., 0.] if args.connections == 'intra' else [0., 0., 0., 0.]
    ...
    for model_idx in range(args.num_model):
        ...
        each_layer_avg_fr += test_mujoco_render(model_file_dir, 
                                                buffer_file_dir, 
    ...
    for i, avg_fr in enumerate(each_layer_avg_fr):
        print('The average fire-rate of Layer %d: %.1f%%' % (i + 1, 100 * avg_fr / args.num_model))
